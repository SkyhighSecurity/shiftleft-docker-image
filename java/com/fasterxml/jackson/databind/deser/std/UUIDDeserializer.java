...
/*     */ 
/*     */     
/*  46 */     if (id.charAt(8) != '-' || id.charAt(13) != '-' || id
/*  47 */       .charAt(18) != '-' || id.charAt(23) != '-') {
...
/*  84 */     return (byteFromChars(str, index, ctxt) << 24) + (
/*  85 */       byteFromChars(str, index + 2, ctxt) << 16) + (
/*  86 */       byteFromChars(str, index + 4, ctxt) << 8) + 
/*  87 */       byteFromChars(str, index + 6, ctxt);
/*     */   }
/*     */   
...
/* 114 */     throw ctxt.weirdStringException(uuidStr, handledType(), 
/* 115 */         String.format("Non-hex character '%c' (value 0x%s), not valid for UUID String", new Object[] {
/*     */             
/* 117 */             Character.valueOf(c), Integer.toHexString(c) }));
/*     */   }
/*     */   
/*     */   private UUID _fromBytes(byte[] bytes, DeserializationContext ctxt) throws JsonMappingException {
/* 121 */     if (bytes.length != 16) {
/* 122 */       throw InvalidFormatException.from(ctxt.getParser(), "Can only construct UUIDs from byte[16]; got " + bytes.length + " bytes", bytes, 
/*     */           
/* 124 */           handledType());
/*     */     }
...
/* Location:              C:\Users\nateb\Desktop\shiftleft-docker-image-1.2.0.jar!\com\fasterxml\jackson\databind\deser\std\UUIDDeserializer.class
